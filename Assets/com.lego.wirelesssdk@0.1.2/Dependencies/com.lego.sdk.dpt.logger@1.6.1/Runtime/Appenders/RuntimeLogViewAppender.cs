...
namespace LEGO.Logger.Appenders
{
    internal class RuntimeLogViewAppender : MonoBehaviour, ILogAppender
    {
        ...
        [SerializeField] private Button closeRuntimeViewButton;
        ...
        private void Awake()
        {
            ...
            closeRuntimeViewButton.onClick.AddListener(() =>
            {
        ...
        private void InitializeDropDown()
        {
            ...
            {
                var enumName = ((LogLevel) i).ToString();
        ...
        private IEnumerator ListenOnInput()
        {
            while (true)
            {
                ...
                if (Input.GetKey(KeyCode.LeftControl) && Input.GetKeyDown(KeyCode.L))
                    ToggleRuntimeViewActive();
            
                if(Input.touchCount == 0)
                    continue;

                if (Input.GetTouch(0).tapCount >= 5)
                    SetRuntimeViewActive(true);
            }
        }

        private void SetRuntimeViewActive(bool activeState)
        {
            gameObject.SetActive(activeState);
        }

        private void ToggleRuntimeViewActive()
        {
