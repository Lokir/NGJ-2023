...
namespace LEGODeviceUnitySDK
{
    public class LEGODevice : AbstractLEGODevice, LegoMessage_Visitor<object>
    {
        ...
        internal LEGODevice(IBleDevice bleDevice, ILEGODeviceConfig lEGODeviceConfig)
            : base(bleDevice, lEGODeviceConfig)
        {
        ...
        {
            this.interrogationState = new DeviceInterrogationState(this, lEGODeviceConfig, UpdateDeviceState);
        }
        ...
        internal override void ReceivedPacket(byte[] data)
        {
            ...
            {
                logger.Warn( $" Got packet while in state:{ConnectionState}: data:{BitConverter.ToString(data)}");
            }
        ...
        async void LegoMessage_Visitor<object>.handle_MessageHubAction(MessageHubAction msg, object arg)
        {
            ...
            case HubAction.WILL_SWITCH_OFF:
                ...
                {   //:NOTE: Technic (hub128) hub react differently from the preceding hubs in that they
                    // don't issue a WILL_DISCONNECT after being told to shut down and will go into advertising mode if
                    // we attempt to disconnect them before they disconnect themselves without telling us. Craptastic
                    // - Anders Bang is raising a bugfix on PT - This code will need to be removed when this is fixed
                    // - also make sure the "async" in the method signature above is removed
                    await Task.Delay(850); // give the hub chance for it to disconnect before we do. NB: 750 is too little
                    Disconnect();
                }
                if (deviceType.Equals(DeviceType.Hub66))
                {
                    // give the hub chance for it to disconnect before we do and
                    // allow the hub to stop advertising in order to avoid immediate auto reconnect
