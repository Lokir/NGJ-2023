...
namespace LEGODeviceUnitySDK
{
    ...
    public class DeviceInterrogationState {
        ...
        public DeviceInterrogationState(LEGODevice device, ILEGODeviceConfig lEGODeviceConfig, Action onComplete)
        {
            legoDevice = device;
            retryCount = 0;
            ...
            interrogationTimer = new ConnectInterrogationTimer(LEGODeviceManager.Instance,
                ...
                success => OnInterrogationFinishedAction(success, lEGODeviceConfig),
                lEGODeviceConfig);
        ...
        private void OnInterrogationFinishedAction (bool success, ILEGODeviceConfig lEGODeviceConfig)
        {
            // if failed and the client wants to "hear" about it
            if (!success && lEGODeviceConfig.IgnoreFailedInterrogationEvents == false) 
            {
