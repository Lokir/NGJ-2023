...
namespace LEGODeviceUnitySDK
{
    ...
    public abstract class LEGOServiceCommand : ICommand
    {
        ...
        protected byte EncodeLedMatrixLevel(int v) {
            return (byte)Mathf.Clamp(v, -9, 9);
        }
        protected byte EncodeLedMatrixColor(int v) {
            return (byte)Mathf.Clamp(v, 0, 10);
        }
        protected byte EncodeLedMatrixTransitionColor(int v) {
            return (byte)Mathf.Clamp(v, 0, 10);
        }
        protected byte EncodeLedMatrixTransitionType(int v) {
            return (byte)Mathf.Clamp(v, 0, 2);
        }
        protected byte EncodeLedMatrixColorAndIntensity(LEGOLEDMatrix.PixelConfiguration v)
        {
            var config = (byte) (v.Color + (v.Intensity << 4));
            return (byte) Mathf.Clamp(config, 0, 170);
        }
